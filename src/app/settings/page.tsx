...
function SettingRow({ label, sublabel, children }: { label: string; sublabel?: string; children?: React.ReactNode }) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 py-3 border-b border-slate-50 last:border-0">
      <div>
        ...
        <form onSubmit={handleSubmit} className="px-5 py-5 space-y-4">
          <div>
            ...
            <input value={name} onChange={(e) => setName(e.target.value)}
              className="w-full text-sm border border-slate-200 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500" required />
          </div>
          <div>
          ...
          <div>
            ...
            <select value={role} onChange={(e) => setRole(e.target.value as Agent["role"])}
              className="w-full text-sm border border-slate-200 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500">
          ...
          <div className="flex gap-3 pt-2 pb-2">
            <button type="button" onClick={onClose}
              className="flex-1 py-2.5 text-sm font-medium text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50">취소</button>
            <button type="submit"
              className="flex-1 py-2.5 text-sm font-medium text-white bg-brand-500 rounded-lg hover:bg-brand-600">저장</button>
          </div>
        </form>
      </div>
    </div>
      ...
      <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-xl w-full sm:max-w-md">
        ...
        <form onSubmit={handleSubmit} className="px-5 py-5 space-y-4">
          ...
          <div>
            ...
            <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="공인중개사"
              className="w-full text-sm border border-slate-200 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500" required />
          </div>
          <div>
            <label className="text-xs font-medium text-slate-600 block mb-1.5">권한</label>
          ...
          <div className="flex gap-3 pt-2 pb-2">
            <button type="button" onClick={onClose}
              className="flex-1 py-2.5 text-sm font-medium text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50">취소</button>
            <button type="submit"
              className="flex-1 py-2.5 text-sm font-medium text-white bg-brand-500 rounded-lg hover:bg-brand-600">추가</button>
          </div>
        </form>
      </div>
    </div>
...
function Toast({ message, onDone }: { message: string; onDone: () => void }) {
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 bg-slate-800 text-white text-sm px-4 py-2.5 rounded-xl shadow-lg whitespace-nowrap">
...
export default function SettingsPage() {
  ...
  const handleEdit = (data: Pick<Agent, "name" | "title" | "role">) => {
    ...
    const updated = agents.map((a) => a.id === editTarget.id ? { ...a, ...data } : a);
    save(updated);
    setEditTarget(null);
  ...
  const handleAdd = (data: Omit<Agent, "id" | "properties" | "transactions" | "commission">) => {
    const newAgent: Agent = { ...data, id: Date.now().toString(), properties: 0, transactions: 0, commission: 0 };
      ...
      <div className="max-w-3xl">
        ...
        <SettingSection title="업체 정보" icon={Building2}>
          <SettingRow label="업체명" sublabel="공인중개사무소 상호명">
            <input defaultValue="포스코사랑공인중개사"
              className="w-full sm:w-64 text-sm border border-slate-200 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500" />
          </SettingRow>
          <SettingRow label="등록번호" sublabel="공인중개사 등록 번호">
            <input defaultValue="중구-1609"
              className="w-full sm:w-64 text-sm border border-slate-200 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500" />
          </SettingRow>
          <SettingRow label="사무소 주소" sublabel="대전 중구 목동">
            <input defaultValue="대전 중구 목동로 22번길 16"
              className="w-full sm:w-64 text-sm border border-slate-200 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500" />
          </SettingRow>
          <SettingRow label="대표 전화" sublabel="사무소 연락처">
            <input defaultValue="042-223-2300"
              className="w-full sm:w-64 text-sm border border-slate-200 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500" />
          </SettingRow>
        </SettingSection>
        ...
        <SettingSection title="계정 관리" icon={User}>
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <div className="flex items-center gap-1">
                      <span className={cn("text-xs font-medium hidden sm:block", agent.active ? "text-emerald-600" : "text-slate-400")}>
                        {agent.active ? "활성" : "비활성"}
                      </span>
                      <Toggle checked={agent.active} onChange={() => toggleActive(agent.id)} />
                    </div>
                    <button onClick={() => setEditTarget(agent)}
                      className="flex items-center gap-1 px-2 py-1.5 text-xs font-medium text-brand-600 border border-brand-200 rounded-lg hover:bg-brand-50">
                      <Pencil size={12} />
                      <span className="hidden sm:inline">수정</span>
                    </button>
                    <button onClick={() => setDeleteId(agent.id)}
                      className="flex items-center gap-1 px-2 py-1.5 text-xs font-medium text-red-500 border border-red-200 rounded-lg hover:bg-red-50">
                      <Trash2 size={12} />
          ...
          <button onClick={() => setShowAdd(true)}
            className="mt-3 w-full py-2.5 text-sm text-brand-600 border border-brand-200 border-dashed rounded-lg hover:bg-brand-50 flex items-center justify-center gap-2">
            <Plus size={15} />
            직원 추가
          </button>
        </SettingSection>
        ...
        <SettingSection title="보안 설정" icon={Shield}>
          <SettingRow label="비밀번호 변경" sublabel="마지막 변경: 2026-01-15">
            <button className="w-full sm:w-auto text-sm text-brand-600 border border-brand-200 px-3 py-2 rounded-lg hover:bg-brand-50">변경</button>
          </SettingRow>
          ...
          <SettingRow label="세션 타임아웃" sublabel="비활성 후 자동 로그아웃">
            <select className="w-full sm:w-auto text-sm border border-slate-200 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500">
        ...
        <SettingSection title="데이터 관리" icon={Database}>
          <SettingRow label="데이터 백업" sublabel="마지막 백업: 2026-05-01 03:00">
            <button onClick={() => showToast("백업이 완료되었습니다.")}
              className="w-full sm:w-auto text-sm text-brand-600 border border-brand-200 px-3 py-2 rounded-lg hover:bg-brand-50">지금 백업</button>
          </SettingRow>
          <SettingRow label="데이터 내보내기" sublabel="거래 내역 및 고객 정보 CSV">
            <button onClick={() => showToast("데이터 내보내기가 시작되었습니다.")}
              className="w-full sm:w-auto text-sm text-slate-600 border border-slate-200 px-3 py-2 rounded-lg hover:bg-slate-50">내보내기</button>
          </SettingRow>
        </SettingSection>
        ...
        <div className="flex flex-col sm:flex-row justify-end gap-3 pb-6">
          <button className="w-full sm:w-auto px-5 py-2.5 text-sm font-medium text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50">취소</button>
          <button onClick={() => showToast("설정이 저장되었습니다.")}
            className="w-full sm:w-auto px-5 py-2.5 text-sm font-medium text-white bg-brand-500 rounded-lg hover:bg-brand-600">저장하기</button>
        </div>
      </div>

      {editTarget && <EditModal agent={editTarget} onSave={handleEdit} onClose={() => setEditTarget(null)} />}
            ...
            <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-xl w-full sm:max-w-sm p-6">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => handleDelete(deleteId)}
                  className="flex-1 py-2.5 text-sm font-medium text-white bg-red-500 rounded-lg hover:bg-red-600">삭제</button>
              </div>
            </div>
          </div>
        );
